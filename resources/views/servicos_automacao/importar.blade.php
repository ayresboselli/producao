			...
			<table class="table table-bordered table-striped table-sm dataTable" width="100%" cellspacing="0">
				...
				@if(count($albuns) > 0)
					...
					<tr>
						...
						<td>{{ $album->os }}</td>
						<td>{{ $album->op }}</td>
                        <td>{{ $album->url_origem }}</td>
                        <td>{{ $album->quantidade }}</td>
                        <td>{{ $album->dt_processo_importacao }}</td>
                        <td>

                        @if($album->tentativa_importar >= 3)
                            <span class="text-danger">Abortado</span>
                        @else
                        ...
                        <td>{{ $album->tentativa_importar }}</td>
                        <td>{{ $album->produto }}</td>
					</tr>
					@endforeach
				@endif
