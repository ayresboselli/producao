        ...
        try:
            ...
            if self.row[7] == 0:
                ...
                if largura != prod_largura or altura != prod_altura:
                    ...
                    '''
                    if prop_produto > prop_imagem:
                        desvio = prop_produto - prop_imagem
                    else:
                        desvio = prop_imagem - prop_produto
                    '''
                    #if desvio > margem_proporcao:
            ...
            sql = "INSERT INTO log(tipo,modulo,mensagem) VALUES(1,'ImportaImagem Contrato','Erro ao importar o arquivo "+str(self.origem)+" para a OS "+str(self.row[8])+"')"
            myConn.execute(sql)
            mydbConn.commit()
            mydbConn.close()
...
myZangraf.execute("UPDATE servicos_status SET ultimo_processo = now() WHERE servico = 'producao-importar'")
mydbZangraf.commit()
...
sql += "WHERE i.url_origem IS NOT NULL AND i.data_importacao IS NULL AND e.tipo_contrato = 1 AND i.tentativa_importar < 3 "
...
for row in result:
    ...
    try:
        ...
        try:
            ...
            sql = "INSERT INTO log(tipo,modulo,mensagem) VALUES(1,'ImportaImagem Contrato','Pasta não encontrada: "+str(path)+"')"
            myZangraf.execute(sql)
            mydbZangraf.commit()
        ...
        if len(pastas) > 0:
            ...
            sql = "UPDATE pedido_items SET dt_processo_importacao = now() WHERE id = "+str(row[0])
            myZangraf.execute(sql)
            mydbZangraf.commit()
            ...
            for pasta in pastas:
                ...
                if os.path.isdir(path+'/'+pasta):
                    ...
                    if len(nome_arq) > 1 and nome_arq[1].lower() == 'jpg':
